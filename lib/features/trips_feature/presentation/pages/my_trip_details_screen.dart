...
class _MyTripDetailsScreenState extends State<MyTripDetailsScreen> {
  ...
  Future<void> _initialFetch(TripsCubit cubit) async {
    if (_didInitialLoad) return;
    final t = _token;
    if (t == null || t.isEmpty) return;
    _didInitialLoad = true;
    await cubit.fetchTripDetails(tripId: widget.tripId, token: t);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider<TripsCubit>(
      ...
      child: Builder(
        builder: (context) {
          // Trigger initial load after provider exists and after first frame.
          WidgetsBinding.instance.addPostFrameCallback((_) {
            _initialFetch(context.read<TripsCubit>());
          });
                    ...
                    child: Scaffold(
                      ...
                      body: RefreshIndicator(
                        onRefresh: () async {
                          final t = _token;
                          if (t == null || t.isEmpty) return;
                          await context.read<TripsCubit>().fetchTripDetails(tripId: widget.tripId, token: t);
                        },
                ...
                orElse: () {
                  return Directionality(
                    ...
                    child: Scaffold(
                      appBar: const BackHeader(),
                      backgroundColor: AppColors.backgroundColor,
                      body: const Center(child: DTLoading()),
                    ),
                  );
                },
              );
              ...
              child: Column(
                ...
                children: [
                  ...
                  AppButton(
                    lbl: "تأكيد",
                    onPressed: () {
                      tripsCubit.cancelTicket(tripId: int.parse(tripId), token: token);
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child: Text('حسنًا', style: TextStyle(fontSize: width * 0.04)),
          ),
        ],
      ),
    );
